...
def retrieve():
    ...
    try:
        response = requests.get(f"{BACKEND_API_URL}/api/devices", cookies=request.cookies)
        response.raise_for_status()
...
def proposals_proxy():
    try:
        resp = api_request('GET', f"/api/baseline/proposals{request.query_string.decode() and '?' + request.query_string.decode()}")
...
@login_required
    ...
    """Proxy endpoint for fetching approvals from backend"""
    try:
        response = api_request('GET', '/api/approvals')
...
@login_required
def save_baseline_proposal_proxy(proposal_id):
    """Proxy for manually saving a baseline proposal to the backend."""
    try:
        response = api_request('PUT', f'/api/baseline/proposals/{proposal_id}', json=request.get_json())
        response.raise_for_status()
...
def save_backup_proxy():
    ...
    try:
        response = api_request('POST', '/api/backups/save', json=request.get_json())
        response.raise_for_status()
...
def delete_device(device_name):
    ...
    try:
        response = api_request('DELETE', f'/api/device/{device_name}')
...
def devices():
    ...
    return render_template("devices.html", active_tab='devices', devices=devices_list, error=error)
